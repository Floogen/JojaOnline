...
namespace JojaOnline
{
    public static class JojaResources
    {
        ...
        {
            string assetFolderPath = helper.Content.GetActualAssetKey("assets", ContentSource.ModFolder);
            // Load the MFM related background(s)
            jojaMailBackground = helper.Content.Load<Texture2D>(Path.Combine(assetFolderPath, "jojaLetterBG.png"));

            // Load in the JojaSite background
            jojaSiteBackground = helper.Content.Load<Texture2D>(Path.Combine(assetFolderPath, "jojaStoreBG.png"));

            // Load in the JojaSite spritesheet
            jojaSiteSpriteSheet = helper.Content.Load<Texture2D>(Path.Combine(assetFolderPath, "jojaSiteSprites.png"));

            // Load JojaSite checkout background
            jojaCheckoutBackground = helper.Content.Load<Texture2D>(Path.Combine(assetFolderPath, "jojaCheckoutBG.png"));

            // Load JojaSite ad banners
            jojaAdBanners = helper.Content.Load<Texture2D>(Path.Combine(assetFolderPath, "jojaBanners.png"));

            // Load JojaSite's app icon for Mobile Phone mod
            jojaAppIcon = helper.Content.Load<Texture2D>(Path.Combine(assetFolderPath, "mobile", "jojaIcon.png"));

            // Load JojaSite's app vertical background for Mobile Phone mod
            jojaMobileVertBackground = helper.Content.Load<Texture2D>(Path.Combine(assetFolderPath, "mobile", "jojaMobileVerticalBG.png"));

            // Load JojaSite's app horizontal background for Mobile Phone mod
            jojaMobileHorzBackground = helper.Content.Load<Texture2D>(Path.Combine(assetFolderPath, "mobile", "jojaMobileHorizontalBG.png"));
        }
